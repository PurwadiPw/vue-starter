...
export default {
  ...
  data() {
    ...
    this.$stateMerge(data, {
      ...
      delRowsRoute: 'permissionDelBatch',
      restoreRowsRoute: 'permissionRestoreBatch',
    })
    return data
  },
  created() {
  ...
  methods: {
    ...
    actionRowClicked(command) {
      ...
      setTimeout(() => {
        if (command.mode === 'editRow' || command.mode === 'cloneRow') {
          ...
          // vm.$refs[`${vm.gridName}RowAction${command.rowData.id}`].hide()
          const route = `api/permission/${command.rowData.id}/audit`
          vm.viewMode = 'audit'
          vm.$nextTick().then(() => {
    ...
    getMenuItems() {
      return (this.$refs[this.formName].formDataEdit !== null) ? [this.$refs[this.formName].formDataEdit.menu] : null
    }
  }
};
</script>
